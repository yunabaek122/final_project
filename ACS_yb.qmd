...
cleaned_poverty_shp = poverty_merged_shp[['ZIPCODE', 'poverty', 'geometry']]
cleaned_poverty_shp.to_file("cleaned_poverty.shp")

print(cleaned_poverty_shp['poverty'].dtype) 
cleaned_poverty_shp['poverty'] = pd.to_numeric(cleaned_poverty_shp['poverty'], errors='coerce')
...
# Step 2: Filter rows with ZIP code information
poverty_df = poverty_df[poverty_df['Label (Grouping)'].str.startswith('ZCTA5')]

# Step 3: Extract ZIP codes
poverty_df['ZIPCODE'] = poverty_df['Label (Grouping)'].str.extract(r'ZCTA5 (\d{5})')[0].astype(int)

# Step 4: Rename the chosen column
poverty_df.rename(columns={
    'Population for whom poverty status is determined!!ALL INDIVIDUALS WITH INCOME BELOW THE FOLLOWING POVERTY RATIOS!!50 percent of poverty level': 'poverty_rate'
}, inplace=True)

# Step 5: Convert to numeric
poverty_df['poverty_rate'] = poverty_df['poverty_rate'].str.replace(',', '').astype(float, errors='coerce')

# Step 6: Keep only relevant columns
poverty_cleaned = poverty_df[['ZIPCODE', 'poverty_rate']]
